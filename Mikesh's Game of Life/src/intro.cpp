...
void Intro::DrawLogoScreen()
{
    ...
    else if (state == 3)    // Draw "raylib" text-write animation + "powered by"
    {
        ...
        if (framesCounter > 20) DrawText("powered by", logoPositionX, logoPositionY - 27, 20, Fade(DARKGRAY, alphaPowered));
        if (framesCounter > 50) DrawText("Mikesh's Game of Life", GetScreenWidth() / 2 - MeasureText("Mikesh's Game of Life",60) / 2, logoPositionY + 300, 60, Fade(BLACK, alphaTitle));
    }
}

bool Intro::IsIntroFininsh()
{
    return finishScreen;
}
