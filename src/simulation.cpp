...
void
Simulation::SetCellValue (int row, int column, int value)
{
  grid.SetValue (row, column, value);
}

int
Simulation::CountLiveNeighbours (int row, int column)
{
  int liveNeighbours = 0;
  vector<pair<int, int> > neighbourOffsets = {
    { -1, 0 },  // Directly above
    { 1, 0 },   // Directly below
    { 0, -1 },  // To the left
    { 0, 1 },   // To the right
...
void
Simulation::ToggleCell (int row, int column, bool state)
{
...
bool
Simulation::IsRandomColors ()
{
  return grid.IsRandomColors ();
}

void
Simulation::SetEdges (int edge)
{
  grid.SetEdges (edge);
...
void
Simulation::SetColor (bool isLiveColor, int red, int green, int blue,
                      int alpha)
{
  if (isLiveColor)
    {
...
void
Simulation::SetRandomSize (int value)
{
