      ...
      for (int column = 0; column < columns; column++)
        {
          ...
          DrawRectangle (column * cellSize, row * cellSize + 30,
                         cellSize - edgesWidth, cellSize - edgesWidth, color);
        }
    }
}
...
void
Grid::ToggleCell (int row, int column, bool state)
{
  if (IsWithinBounds (row, column))
    {
...
void
Grid::SetColorLive (int red, int green, int blue, int alpha)
{
...
void
Grid::SetColorDead (int red, int green, int blue, int alpha)
{
...
int
Grid::AdjustColorComponent (int colorComponent, int randomSize)
{
  randomSize = clamp (randomSize, 0, 127);
...
Color
Grid::SetColor (int red, int green, int blue, int alpha)
{
  Color color{};
  color.r = clamp (red, 0, 255);
  color.g = clamp (green, 0, 255);
  color.b = clamp (blue, 0, 255);
